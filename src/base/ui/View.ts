namespace app.ui {
    ...
    export class View extends Laya.View {
        ...
        protected _modelEvents: { dispatcher: Laya.EventDispatcher, events: string[] }[] | string[];
        ...
        protected registerModelEvents(isRegister: boolean) {
            let modelEvents = this._modelEvents as any;
            if (modelEvents && modelEvents.length > 0) {
                if (typeof modelEvents[0] == "string") {
                    bindModelEvents.call(this, { dispatcher: modelEventsDispatcher, events: modelEvents }, isRegister);
                } else {
                    modelEvents.forEach((eventGroup: any) => bindModelEvents.call(this, eventGroup, isRegister));
                }
            }

            function bindModelEvents(this: any, eventGroup: { dispatcher: Laya.EventDispatcher, events: string[] }, isRegister: boolean) {
                eventGroup.events.forEach(event => {
                    let method = this[event];
                    if (method) {
                        if (isRegister) {
                            eventGroup.dispatcher.on(event, this, method);
                        } else {
                            eventGroup.dispatcher.off(event, this, method);
                        }
                    } else {
                        throw 'modelEvents not implemented';
                    }
                });
            }
        }
